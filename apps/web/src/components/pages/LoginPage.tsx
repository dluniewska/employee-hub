        ...
        await login(username, password)
            .then((res) => {
                if (res.status == 200) {
                    ...
                    setErrorMesage("Błąd serwera. Spróbuj ponownie za chwilę")
                }
                else if (res.status >= 400) {
                    setErrorMesage("Niepoprawne dane logowania.")
                }
            })
            ...
            <div className="-mt-16 h-screen w-screen flex justify-center items-center bg-pastel-bg bg-contain flex-1">
                <LoginCard className="container h-fit lg:w-2/5 p-4 sm:w-4/5 flex flex-col justify-center">
                    <CardHeader className="mb-3">
                        ...
                        {errorMessage && <CardDescription className="text-red-700">{errorMessage}</CardDescription>}
                    </CardHeader>
                    <CardContent>
                        <form>
